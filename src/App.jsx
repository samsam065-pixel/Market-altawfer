...
return(<div style={{...BG,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"16px",position:"relative",overflow:"hidden"}}><style>{CSS}</style><div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"8px"}}><button onClick={()=>setScreen("levelSelect")} style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:"10px",padding:"6px 12px",color:"rgba(255,255,255,0.7)",fontSize:"0.8rem",cursor:"pointer"}}>🗺️ المراحل</button><h1 style={{fontSize:"1.4rem",fontWeight:900,margin:0,background:"linear-gradient(90deg,#FF6B9D,#A855F7,#60a5fa)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>🛒 Market Altawfer</h1></div><div style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(168,85,247,0.3)",borderRadius:"12px",padding:"5px 18px",marginBottom:"10px",color:"#A855F7",fontWeight:700,fontSize:"0.82rem"}}>المرحلة {currentLevel+1} / 15 — {lv.label}</div><div style={{display:"flex",gap:"10px",marginBottom:"8px"}}>{[{label:"النقاط",value:score,color:"#FFD700"},{label:"الهدف",value:lv.targetScore,color:"#10B981"},{label:"الحركات",value:moves,color:moves<=5?"#FF4444":"#60a5fa"}].map(({label,value,color})=>(<div key={label} style={{background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:"12px",padding:"6px 12px",textAlign:"center"}}><div style={{color:"rgba(255,255,255,0.5)",fontSize:"0.62rem",marginBottom:"2px"}}>{label}</div><div style={{color,fontSize:"1.1rem",fontWeight:800}}>{value}</div></div>))}</div><div style={{width:"100%",maxWidth:"390px",height:"9px",background:"rgba(255,255,255,0.1)",borderRadius:"99px",marginBottom:"10px",overflow:"hidden"}}><div style={{height:"100%",width:progress+"%",background:"linear-gradient(90deg,#FF6B9D,#A855F7,#60a5fa)",borderRadius:"99px",transition:"width 0.4s",boxShadow:"0 0 8px rgba(168,85,247,0.5)"}}/></div>{showCombo&&(<div style={{position:"fixed",top:"25%",left:"50%",transform:"translateX(-50%)",zIndex:100,fontSize:"3rem",fontWeight:900,color:"#FFD700",textShadow:"0 0 20px #FF6B9D,0 0 40px #A855F7",animation:"comboAnim 1.2s forwards",pointerEvents:"none"}}>🔥 COMBO x{combo}!</div>)}<div style={{display:"grid",gridTemplateColumns:`repeat(${COLS},1fr)`,gap:"3px",background:"rgba(255,255,255,0.05)",border:"2px solid rgba(168,85,247,0.4)",borderRadius:"18px",padding:"10px",boxShadow:"0 0 40px rgba(168,85,247,0.3),inset 0 0 30px rgba(0,0,0,0.3)"}}>{board&&board.map((row,r)=>row.map((candy,c)=>{const mat=isMatched(r,c),sel=isSelected(r,c),hin=isHint(r,c);const bg=candy?COLORS[candy]:"transparent";return(<div key={`${r}-${c}`} className={`cc${sel?" sel":""}${mat?" mat":""}${hin?" hin":""}`} onClick={()=>handleCellClick(r,c)} style={{width:"42px",height:"42px",borderRadius:"10px",background:sel?`radial-gradient(circle,#fff 0%,${bg} 60%)`:`radial-gradient(circle at 35% 35%,rgba(255,255,255,0.5) 0%,${bg} 55%,rgba(0,0,0,0.2) 100%)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.3rem",cursor:"pointer",userSelect:"none",boxShadow:sel?"0 0 12px #fff,0 4px 8px rgba(0,0,0,0.4)":"0 3px 6px rgba(0,0,0,0.3)",border:sel?"2px solid #fff":"1px solid rgba(255,255,255,0.15)",transition:"all 0.15s",animation:mat?"pop 0.4s forwards":"none"}}>{candy}</div>);}))}</div><div style={{display:"flex",gap:"10px",marginTop:"12px"}}><button onClick={()=>startLevel(currentLevel)} style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:"12px",padding:"7px 16px",color:"rgba(255,255,255,0.7)",fontSize:"0.85rem",cursor:"pointer"}}>🔄 إعادة</button><button onClick={handleHint} disabled={hintsLeft<=0||animating} style={{background:hintsLeft>0?"linear-gradient(135deg,#FFD700,#FF6B9D)":"rgba(255,255,255,0.05)",border:hintsLeft>0?"none":"1px solid rgba(255,255,255,0.1)",borderRadius:"12px",padding:"7px 16px",color:hintsLeft>0?"#1a0533":"rgba(255,255,255,0.3)",fontSize:"0.85rem",fontWeight:700,cursor:hintsLeft>0?"pointer":"not-allowed",boxShadow:hintsLeft>0?"0 0 12px rgba(255,215,0,0.4)":"none"}}>💡 مساعدة ({hintsLeft}/2)</button></div><p style={{color:"rgba(255,255,255,0.25)",fontSize:"0.68rem",marginTop:"8px"}}>طابق 3 أو أكثر • اجمع {lv.targetScore} نقطة لتكمل المرحلة 🛒</p></div>);
}
